...
void Intrinsics::setup(float focalLength, cv::Size imageSize, cv::Size2f sensorSize, cv::Point2d principalPoint) {
   ...
   cv::Mat cameraMatrix = (cv::Mat1d(3, 3) << fx, 0, cx,
                           0, fy, cy,
                           0, 0, 1);
...
void Calibration::loadLcp(const std::string& filename, float focalLength, int imageWidth, int imageHeight, bool absolute) {
   ...
   if (imageWidth == 0)
      imageWidth = lcpImageWidth;
   if (imageHeight == 0)
      imageHeight = lcpImageHeight;
   ...
   if (patternType == CHESSBOARD) {
      ...
      if (found) {
         ...
         if (refine) {
            ...
            cornerSubPix(grayMat, pointBuf, subpixelSize, cv::Size(-1, -1), cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 30, 0.1));
         }
      }
   }
